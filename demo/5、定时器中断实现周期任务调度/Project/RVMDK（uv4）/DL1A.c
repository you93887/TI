...
void DelA_PinInit()
{
    SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOA);	    // 使能GPIO端口时钟
    SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOD);
    while(!SysCtlPeripheralReady(SYSCTL_PERIPH_GPIOA));    // 等待端口时钟就绪
    // 配置SDA和SCL引脚为输出
    GPIOPinTypeGPIOOutput(DL1A_SDA_PORT, DL1A_SDA_PIN);
    GPIOPinTypeGPIOOutput(DL1A_SCL_PORT, DL1A_SCL_PIN);
	  GPIOPinTypeGPIOOutput(DL1A_XSHUT_PORT,DL1A_XSHUT_PIN);
}

#define GET_DL1A_SDA   		 		    GPIOPinRead(DL1A_SDA_PORT, DL1A_SDA_PIN)
#define DL1A_SDA_LOW()         		GPIOPinWrite(DL1A_SDA_PORT, DL1A_SDA_PIN, 0)    // IO口输出低电平
#define DL1A_SDA_HIGH()        		GPIOPinWrite(DL1A_SDA_PORT, DL1A_SDA_PIN, DL1A_SDA_PIN)  // IO口输出高电平
#define DL1A_SCL_LOW()          	GPIOPinWrite(DL1A_SCL_PORT, DL1A_SCL_PIN, 0)    // IO口输出低电平
#define DL1A_SCL_HIGH()         	GPIOPinWrite(DL1A_SCL_PORT, DL1A_SCL_PIN, DL1A_SCL_PIN)  // IO口输出高电平
#define ack 1      //主应答
#define no_ack 0   //从应答	
...
static int dl1a_sccb_waitack(void)
{
  DL1A_SCL_LOW();
	...
	DL1A_SCL_HIGH();
  dl1a_simiic_delay();
	GPIOPinTypeGPIOInput(DL1A_SDA_PORT, DL1A_SDA_PIN);
  if(GET_DL1A_SDA==DL1A_SDA_PIN)           //应答为高电平，异常，通信失败
    {
