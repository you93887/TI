...
void OLED_W_SCL(uint8_t BitValue)
{
	...
	if(BitValue==1)
	GPIOPinWrite(GPIO_OLED,GPIO_PIN_4,GPIO_PIN_4);
	else
	GPIOPinWrite(GPIO_OLED,GPIO_PIN_4,0);
...
void OLED_W_SDA(uint8_t BitValue)
{
	...
	if(BitValue==1)
	GPIOPinWrite(GPIO_OLED,GPIO_PIN_5,GPIO_PIN_5);
	else
	GPIOPinWrite(GPIO_OLED,GPIO_PIN_5,0);
	/*如果单片机速度过快，可在此添加适量延时，以避免超出I2C通信的最大速度*/
	//...
}
