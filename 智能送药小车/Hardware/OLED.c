...
void OLED_W_SCL(uint8_t BitValue)
{
	...
	if(BitValue==1)
	{
		GPIOPinWrite(GPIO_OLED, GPIO_OLED_SCL, GPIO_OLED_SCL);
		//GPIOPinWrite(GPIO_PORTB_BASE, GPIO_PIN_0, BitValue);
	}
	else
		GPIOPinWrite(GPIO_OLED, GPIO_OLED_SCL, 0);
	//Delay_Us(50);
...
//	GPIOPinWrite(GPIO_OLED, GPIO_OLED_SDA, BitValue);
	if(BitValue==1)
	{
		GPIOPinWrite(GPIO_OLED, GPIO_OLED_SDA, GPIO_OLED_SDA);
		//GPIOPinWrite(GPIO_PORTB_BASE, GPIO_PIN_1, BitValue);
	
	}
	else
		GPIOPinWrite(GPIO_OLED, GPIO_OLED_SDA, 0);
...
void OLED_GPIO_Init(void)
{
  ...
  GPIOPinTypeGPIOOutput(GPIO_OLED, GPIO_OLED_SCL);
  GPIOPinTypeGPIOOutput(GPIO_OLED, GPIO_OLED_SDA);
//  
//	SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOB);// Enable the GPIO port that is used for the on-board LED.
