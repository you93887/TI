...
void Timer_init(void)//系统调度定时器初始化
{
  ...
  ROM_TimerLoadSet(TIMER0_BASE,TIMER_A,SysCtlClockGet()/1000);		//设定装载值,（80M/1000）*1/80M=1ms				
...
{
	 ROM_SysCtlPeripheralEnable(SYSCTL_PERIPH_TIMER1);					//定时器0使能	  可以查看括号里面的参数看有哪些参数	
  ROM_TimerConfigure(TIMER1_BASE,TIMER_CFG_PERIODIC);						//32位周期定时器			  查看函数定义有参数选择 如向上计数   	
  ROM_TimerLoadSet(TIMER1_BASE,TIMER_A,SysCtlClockGet()/10);		//设定装载值,（80M/10）*1/80M=100ms				
  ROM_IntEnable(INT_TIMER1A);										//总中断使能			右键查看参数位置可知响应的头文件和类似的参数	
  ROM_TimerIntEnable(TIMER1_BASE,TIMER_TIMA_TIMEOUT); 					//中断输出, 设置模式;	查看函数定于里面有模式选择		
  TimerIntRegister(TIMER1_BASE,TIMER_A,TIMER1A_Handler);		//中断函数注册  第三个参数为中断反馈寒素 
  ROM_IntMasterEnable();			
  ROM_TimerEnable(TIMER1_BASE,TIMER_A); 												//定时器使能开始计数
  ROM_IntPrioritySet(INT_TIMER1A, 0xA0);    //设置中断优先级   第一个参数 ：指定要设置优先级的中断。   第二个参数：指定中断的优先级  如0xE0
